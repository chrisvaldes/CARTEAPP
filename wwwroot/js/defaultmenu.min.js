...
function setNavActive() {
    // Clear previous active/open states to avoid duplicates
    document.querySelectorAll('.main-menu .active').forEach((el) => el.classList.remove('active'));
    document.querySelectorAll('.main-menu .open').forEach((el) => el.classList.remove('open'));
    document.querySelectorAll('.slide-menu').forEach((ul) => (ul.style.display = 'none'));

    // current location normalized
    const origin = location.origin;
    const currentPath = (location.pathname || '/').split('?')[0];
    const normalized = currentPath.replace(/^\/+|\/+$/g, '') || 'index.html';
    ...
    sidemenuItems.forEach((el) => {
        ...
        if (!hrefRaw && !dataRoute) return;
        if (hrefRaw.indexOf('javascript') === 0 || hrefRaw === '#') return;

        // prefer explicit data-route when provided by server for exact matching
        let comparePath = '';
        try {
            if (dataRoute) comparePath = new URL(dataRoute, origin).pathname;
            else comparePath = new URL(hrefRaw, origin).pathname;
        } catch (err) {
            // fallback to raw value
            comparePath = dataRoute || hrefRaw;
        }

        const hrefNorm = comparePath.replace(/^\/+|\/+$/g, '') || 'index.html';

        // match strategy
        const normLast = normalized.substring(normalized.lastIndexOf('/') + 1);
        const hrefLast = hrefNorm.substring(hrefNorm.lastIndexOf('/') + 1);

        let isMatch = false;
        // If server provided explicit data-route we require stricter matching (exact or prefix)
        if (dataRoute) {
            isMatch =
                hrefNorm === normalized ||
                normalized.startsWith(hrefNorm + '/') ||
                hrefNorm.startsWith(normalized + '/');
        } else {
            // fallback heuristics for static links: exact, last segment or contains
            isMatch =
                hrefNorm === normalized ||
                hrefNorm === normLast ||
                hrefLast === normLast ||
                normalized.endsWith('/' + hrefNorm) ||
                hrefNorm.endsWith('/' + normalized) ||
                (hrefNorm.length > 0 && normalized.indexOf(hrefNorm) !== -1);
        }
        ...
        if (isMatch) {
            ...
            // open parent ULs and LI.has-sub ancestors
            let parentUl = el.closest('ul');
            while (parentUl && !parentUl.classList.contains('main-menu')) {
                ...
                if (parentUl.classList.contains('slide-menu')) parentUl.style.display = 'block';
                if (parentUl.classList.contains('child1')) slideDown(parentUl);

                parentUl = parentUl.parentElement.closest('ul');
            }

            let ancestorLi = el.closest('li');
            while (ancestorLi) {
                if (ancestorLi.classList.contains('has-sub')) {
                    ancestorLi.classList.add('open', 'active');
                    const firstItem = ancestorLi.querySelector(':scope > .side-menu__item');
                    if (firstItem) firstItem.classList.add('active');
                    const submenu = ancestorLi.querySelector(':scope > .slide-menu');
